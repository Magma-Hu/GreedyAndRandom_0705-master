...
def algorithm1(parents1,offspring1,temperature,population1):
    ...
    if delta_cost > 0 or random.random() < sigmoid(delta_cost / temperature):#随着轮次增加变得更容易接受新解？temp变小，delta变大，这合理吗，应该设置成大于。
        '''for m in offspring1:
            population1.append(m)
        print('?',population1==population_temp)'''
        #print('接受新解')
...
def save_user(now_generation,input_data,name):
    ...
    savepath=f'D:/python-code/GreedyAndRandom_0705-master/recombine/model_save/{name}/temperature_500_{name}_{now_generation}_{timestamp}.npy'
    np.save(savepath,input_data)

def load_user(loadpath,input):
    return np.load(loadpath)
...
# 初始化选择
    fitness_values1 = fitness_func(population1,nodes,blocks)
    fitness_values2 = fitness_func(population2,nodes,blocks)
    #print('???',fitness_values2==fitness_values1)
...
with open('D:/python-code/GreedyAndRandom_0705-master/recombine/results_mine/best_fitness11_mean_1_main3_3.0.txt', 'a') as f:
                    f.write('{0}\n'.format(best_fitness1_mean))
                    f.close()
with open('D:/python-code/GreedyAndRandom_0705-master/recombine/results_mine/best_fitness1_main3_3.0.txt', 'a') as f:
                    f.write('{0}\n'.format(best_fitness1))
                    f.close()
with open('D:/python-code/GreedyAndRandom_0705-master/recombine/results_mine/best_fitness22_mean_2_main3_3.0.txt', 'a') as f:
                    f.write('{0}\n'.format(best_fitness2_mean))
                    f.close()
with open('D:/python-code/GreedyAndRandom_0705-master/recombine/results_mine/best_fitness2_main3_3.0.txt', 'a') as f:
...
population = create_population(block_node_alloDict, AlgorithmList, population_size)
# 初始化父代
fitness_values = fitness_func(population,nodes,blocks)
...
for now_generation in range(num_generations):
    ...
    best_fitness1.append(fitness_values[0])
...
''' cost_parent_mean=0
    for j in parents:
        cost_parent_mean+=calculate_nodesCost(j,nodes,blocks)
    cost_parent_mean=cost_parent_mean/(len(parents)) 
    cost_offspring_mean=0
    for i in offspring:
        cost_temp=calculate_nodesCost(i,nodes,blocks)
        cost_offspring_mean+=cost_temp
    cost_offspring_mean=cost_offspring_mean/(len(offspring))

    delta_cost=cost_parent_mean-cost_offspring_mean
    if delta_cost > 0 or random.random() < sigmoid(delta_cost / temperature):#随着轮次增加变得更容易接受新解？temp变小，delta变大，这合理吗，应该设置成大于。
        population[:num_parents] = parents
    population[num_parents:] = offspring #直接用生成的子代替代了父代后面6个= =,感觉可以优化？？毕竟这样效率比较低，又不能保证子代比父代强'''
...
#模拟退火（判断是否可以接受新解，需要计算获得的子代的访问成本）
cost_parent=calculate_nodesCost(block_node_alloDict['myTest'],nodes,blocks)
cost_offspring=calculate_nodesCost(offspring,nodes,blocks)
delta_cost=cost_parent-cost_offspring
