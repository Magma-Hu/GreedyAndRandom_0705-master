...
def main_init(random_seed,nodeNum,blockNum,nodes_layout):
    # gml文件名，由节点数量和节点分布决定
    filename = 'D:/reinforcement-code/GreedyAndRandom_1018-master/GreedyAndRandom_0705-master/Model/NodeInfo/run_count_0/20nodes_50blocks_origin_NodeInfo.gml'.format(nodeNum,nodes_layout)
...
def fitness_func(population,nodes,blocks):
    ...
    for allo in population:
        if allo != []:
        #if len(allo) > 0:
            fitness_values.append(calculate_nodesCost(np.array(allo),nodes,blocks))#为什么分配方案还能为空
...
def selection(population, fitness_values, num_parents):
    ...
    #unique_values = list(set(fitness_values))#转化成列表,因为集合（set）是不包含重复元素的,尊嘟栓q
    unique_values = list(fitness_values)
    sorted_values = sorted(unique_values)#排序
    print(len(population),len(sorted_values),num_parents)
    if len(sorted_values) >= num_parents: #如果排序后适应度值的长度大于等于需要的父代数量
        ...
        if len(sorted_values)>8:
            parents_indices_to_delete=np.array(indices)[8:]
...
#模拟退火（判断是否可以接受新解，需要计算获得的子代的访问成本），模拟退火好像并不能加快速度。。。
    cost_parent_mean=0
    for j in parents:
    ...
    for i in offspring:
        cost_offspring_mean+=calculate_nodesCost(i,nodes,blocks)
...
# 初始化选择
    ...
    parents,parents_indices,population = selection(population, fitness_values, num_parents)


    best_fitness.append(fitness_values[0])
    #print('群体数量',len(population))
...
with open('D:/reinforcement-code/GreedyAndRandom_1018-master/GreedyAndRandom_0705-master/recombine/best_fitness1.txt', 'a') as f:
                    f.write('{0}\n'.format(best_fitness))
                    f.close()
